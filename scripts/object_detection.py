...
# Reading Image
img = cv2.imread("/home/mahmoud/catkin_ws/src/pfa_r2a/dataset/cylinder.jpeg")
...
pt = ( img.shape[1]//2, 3 * img.shape[0] // 4)
for cnt in contours:
    ...
    if len(approx) ==6 :
        ...
        cv2.putText(img,'Cube', pt ,cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, [0,255, 255], 2)
    elif len(approx) == 7:
        print ("Cube")
        cv2.drawContours(img,[cnt],-1,(255,0,0),3)
        cv2.putText(img,'Cube', pt ,cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, [0, 255, 255], 2)
        ...
        print ("Cylinder")
        cv2.drawContours(img,[cnt],-1,(255,0,0),3)
        cv2.putText(img,'Cylinder', pt ,cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, [0, 255, 255], 2)
    elif len(approx) > 10:
        print ("Sphere")
        cv2.drawContours(img,[cnt],-1,(255,0,0),3)
