...
def callback(rgb_data, depth_data, camera_info):
    try:
        ...
        for cnt in contours:
            ...
            point_x,point_y,point_z= coordinates_from_depth(depth_image,inv_fx,inv_fy,m_cx,m_cy,x,y,w,h) 
            ...
            if len(approx) ==6 :
                ...
                n=2
                x1,y1,z1=coordinates_from_depth(depth_image,inv_fx,inv_fy,m_cx,m_cy,int(x-w/2),int(y),w,h) 
                x2,y2,z2=coordinates_from_depth(depth_image,inv_fx,inv_fy,m_cx,m_cy,int(x+w/2),int(y),w,h)
                #print("x1= ",x1)
            ...
            sh.color.b=0
            sh.color.a=1
            if (n>0 and not(math.isnan(point_x) )):
...
def coordinates_from_depth(depth_image,inv_fx,inv_fy,m_cx,m_cy,x,y,w,h) :
    roi_depth = depth_image[int(y-h/2):int(y+h/2), int(x-w/2):int(x+w/2)]
    value = roi_depth.item(int(w/2)+1, int(h/2)+1)
